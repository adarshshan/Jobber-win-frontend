    ...
    <div className="min-h-screen p-3 bg-blue-50">
      <div className="grid md:grid-cols-12 gap-2 sm:gap-5 sm:container md:py-5">
        ...
        <div className="sm:col-span-9 min-h-[100px]  shadow-xl rounded-lg">
          <div className="col-span-12 md:col-span-8 w-full bg-white shadow-lg min-h-[50px] rounded-lg p-4">
            {receivedReq?.length && receivedReq?.length ? (
              receivedReq.map((item, index) => {
                return (
                  <div
                    key={index + Math.random()}
                    className="flex justify-between py-3 p-4 shadow-md"
                  >
                    ...
                    <div className="flex gap-4 text-2xl p-2">
                      <IoMdCloseCircleOutline
                        ...
                        className="mt-3 hover:text-red-600"
                      />
                      <FaCheck
                        onClick={() => handleAddFriend(item._id)}
                        className="mt-3 hover:text-green-400"
                      />
                    </div>
                  </div>
                );
          ...
          {!showFriendsScreen && (
            <div className="w-full bg-white shadow-lg min-h-[50px] rounded-lg mt-4">
              <h1 className="text-lg font-semibold ms-3">
                People you may know
              </h1>
              <div className="flex flex-wrap w-full">
                    ...
                    return (
                      ...
                      >
                        <ContactCard
                          ...
                          setConfirmFriend={setConfirmFriend}
                          sendReq={sendReq}
                          item={item}
                        />
                      </div>
                ...
                {!users?.length && (
                  <h1 className="text-xl font-semibold">No suggessions...</h1>
                )}
              </div>
            </div>
          )}
          {showFriendsScreen &&
            ...
            friendList.map((item, index) => {
              return (
                <div
                  key={index + Math.random()}
                  className="w-full bg-white shadow-lg min-h-[50px] rounded-lg p-4 mt-4"
                >
                  <div className="flex justify-between m-2 p-2 shadow-lg min-[100px]:">
                    ...
                    <div className="flex justify-end gap-3">
                      <button className="font-semibold text-blue-400 mb-4">
                        Message
                      </button>
                      <AlertDialog>
                        <AlertDialogTrigger>
                        ...
                        <AlertDialogContent>
                          ...
                          <AlertDialogFooter>
                            ...
                            <AlertDialogAction>
                              <p onClick={() => handleUnfriend(item._id)}>
                                yes
                              </p>
                            </AlertDialogAction>
                          </AlertDialogFooter>
          ...
          <div className="w-full bg-white shadow-lg min-h-[50px] rounded-lg p-4 mt-4"></div>
        </div>
      </div>
    </div>
  );
