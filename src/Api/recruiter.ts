...
const uploadImg = async (formData: FormData) => {
  try {
    return await Api.post(recruiterRoutes.uploadImg, formData, {
      headers: {
...
const changeStatus = async (status: string, applicationId: string) => {
  try {
    return await Api.put(
      `${recruiterRoutes.changeStatus}${status}/${applicationId}`
    );
  } catch (error) {
    console.log(error as Error);
  }
...
const editJobDetails = async (data: JobInterface, item: any) => {
  try {
    return await Api.put(recruiterRoutes.editJobDetails, {
      data,
      jobId: item._id,
    });
  } catch (error) {
