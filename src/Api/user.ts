import { FormDataInerface } from "../Pages/user/SignupPage";
...
const updateAbout = async (id: string, about: string) => {
  try {
    const result = await Api.put(`${userRoutes.updateAbout}${id}`, {
      aboutInfo: about,
    });
...
const updateUser = async (
  ...
  try {
    if (userId)
      return await Api.put(`${userRoutes.updateUser}${userId}`, {
        name,
        headLine,
        gender,
        qualification,
...
const applyJob = async (
  ...
  try {
    return await Api.post(`${userRoutes.applyJob}${jobId}`, {
      resume,
      qualification,
      experience,
    });
...
const uploadResume = async (formData: FormData) => {
  try {
    return await Api.post(userRoutes.uploadResume, formData, {
      headers: {
...
const createPost = async (pic: string, id: string, caption: string) => {
  try {
    const result = await Api.post(userRoutes.createPost, {
      userId: id,
      imageUrl: pic,
      caption: caption,
    });
...
const sendMessage = async (comment: string, postId: string) => {
  try {
    return await Api.post(`${userRoutes.postCommentsend}${postId}`, {
      comment,
    });
  } catch (error) {
    console.log(error as Error);
